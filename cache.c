...
int main(int argc, char * argv []) {
 ...
 // Process the command line arguments
  ...
  /*
  printf("\texecution time %ld cycles\n", ?);  //number of total cycles, including memory penalties
  printf("\tinstructions %ld\n", ?);			//number of total instructions run
  printf("\tmemory accesses %ld\n", ?);			//number of both loads and stores
  printf("\toverall miss rate %.2f\n", ? );		//number of misses for both loads and stores, divided by number of loads and stores
  printf("\tread miss rate %.2f\n", ? );		//number of load misses divided by number of loads
  printf("\tmemory CPI %.2f\n", ?);				//number of memory penalty cycles, divided by number of instructions
  printf("\ttotal CPI %.2f\n", ?);				//total number of cycles, divided by number of instructions
   printf("\taverage memory access time %.2f cycles\n",  ?);		//number of memory penalty cycles, divided by number of memory accesses
